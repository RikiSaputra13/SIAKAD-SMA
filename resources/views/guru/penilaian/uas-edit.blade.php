...
@section('title', 'Edit Ujian Akhir Semester')
        ...
        <div class="col-md-10">
            <div class="card shadow-sm">
                <div class="card-header bg-warning text-dark">
                    <h5 class="mb-0">
                        <i class="fas fa-edit me-2"></i>Edit Ujian Akhir Semester
                    </h5>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('guru.penilaian.uas.update', $ujian->id) }}" enctype="multipart/form-data" id="formEditUjianUAS">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('deskripsi') is-invalid @enderror" 
                                              ...
                                              placeholder="Deskripsi tentang UAS ini, cakupan materi, kisi-kisi, dll (opsional)">{{ old('deskripsi', $ujian->deskripsi) }}</textarea>
                                    @error('deskripsi')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
...
● Teliti dalam membaca soal dan menjawab">{{ old('instruksi', $ujian->instruksi) }}</textarea>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <a href="{{ route('guru.penilaian.uas.index') }}" class="btn btn-secondary me-2">
                                    <i class="fas fa-arrow-left me-1"></i>Kembali ke Daftar UAS
                                </a>
                                <a href="{{ route('guru.penilaian.uas.show.soal', $ujian->id) }}" class="btn btn-outline-info">
                            ...
                            <div>
                                ...
                                <button type="submit" name="action" value="update" class="btn btn-primary" id="btn-update">
                                    <i class="fas fa-save me-1"></i>Update UAS
                                </button>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="mt-4 pt-3 border-top">
                        <form action="{{ route('guru.penilaian.uas.destroy', $ujian->id) }}" method="POST" class="d-inline" id="deleteForm">
                            @csrf
                            @method('DELETE')
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Form validation
        const form = document.getElementById('formEditUjianUAS');
        ...
        if (btnPublish) {
            btnPublish.addEventListener('click', function(e) {
                ...
                if (!confirm('Ujian Akhir Semester akan dipublish dan dapat diakses siswa. Pastikan semua data sudah benar. Lanjutkan?')) {
                    e.preventDefault();
                    return false;
                }
            });
        ...
        // Update button enhancement for published UAS
        if (btnUpdate) {
            btnUpdate.addEventListener('click', function(e) {
                if (!confirm('PERINGATAN: Mengubah UAS yang sudah dipublish dapat mempengaruhi siswa yang sedang/sudah mengerjakan. Pastikan perubahan benar-benar diperlukan. Lanjutkan?')) {
                    e.preventDefault();
                    return false;
                }
            });
    ...
    function confirmDelete() {
        if (confirm('Apakah Anda yakin ingin menghapus Ujian Akhir Semester ini? Tindakan ini tidak dapat dibatalkan!')) {
